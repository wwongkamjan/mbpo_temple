    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if total_step % 1000 == 0:
                ...
                test_done = False
                while not test_done:
                    test_state, test_action, test_next_state, test_reward, test_done, test_info = env_sampler_test.sample(
                        agent, eval_t=True)
                    sum_reward += test_reward
                logger.info("Step Reward: " + str(total_step) + " " + str(sum_reward))
...
def train_predict_model(args, env_pool, predict_env, clf, clf_r,fitted):
    ...
    batch_sz = 256
    state, action, reward, next_state, done = env_pool.sample(batch_sz)
